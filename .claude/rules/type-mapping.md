...
* **Composition (`allOf`)**: A bare `$ref` member of an `allOf` list becomes an embedded (anonymous) field of the referenced struct type — Go's idiom for composition. A non-`$ref` (inline) `allOf` member instead has its own `properties`/`required` merged directly into the enclosing schema's field set, as if declared there. This applies equally whether the `allOf` is a named `components.schemas` entry or appears anonymously as a property value, an array's items, or an operation's `requestBody`/response schema (see section 4) — an anonymous multi-member `allOf` gets a generated named struct the same way an anonymous `type: object` with its own `properties` does.
...
* **Scope**: `paths.*.parameters`, `paths.*.*.parameters` (see section 6), `paths.*.*.requestBody`, and `paths.*.*.responses` are walked beyond `components.schemas`. A `requestBody` schema generates exactly like any other component reference (plain struct, no special treatment). Responses in the 4xx/5xx range are treated as errors (below); 1xx/2xx/3xx responses and the `default` response key are not — they generate as plain structs, same as `requestBody`.
* **Declaration order**: `requestBody` and 4xx/5xx response schemas are generated in path → operation → status-code order, ahead of the alphabetical `components.schemas` pass — so their declaration order in the output follows the spec's operation order rather than alphabetical schema-name order. An inline (non-`$ref`) 1xx/2xx/3xx/`default` response schema is generated in this same operation order, named `Response<code>` (e.g. `Response200`), exactly like an inline `requestBody` is named `RequestBody` — see the Scope bullet above. A schema-less status code has nothing to generate here. Anything left `$ref`'d to a named component schema and not otherwise touched by an operation (schemas unused by any operation, or a named 2xx/3xx/default schema) still falls through to, and is generated by, the alphabetical `components.schemas` pass afterward.
* **Schema-backed error response**: A 4xx/5xx response with a JSON schema (e.g. `$ref`'d to a component) generates that schema exactly like any other reference, then additionally gets an `Error() string` method (the standard library's `error` interface) on the resolved type.
* **Content-less error response**: A 4xx/5xx response with no `content` (just a `description`, e.g. a bare `"404": {"description": "Not Found"}`) synthesizes an empty struct named `Response<code>` (e.g. `Response404`), with no doc comment, and the same `Error() string` method as above. Reused/deduplicated by status code the same way a repeated enum shape is (see above): declared once even if the same code recurs across operations.
* **Method body**: Every `Error() string` method — schema-backed or content-less — always has the same body: `panic("TODO: define the output")`. The generator has no way to know what the correct output is (which field of an arbitrary schema holds the error message, or what a content-less response should report), so it stubs the body out for the developer to fill in by hand rather than guessing.
...
* **Scope & naming**: An operation's `parameters` (path, query, header, or cookie) generate a single struct named `<PascalCase(operationId)>Params` (e.g. operationId `get-user-by-id` → `GetUserByIdParams`), with a doc comment `<StructName> is generated for operationId <operationId>.` (using the operationId's original, un-Pascal-cased spelling). Parameters declared on the enclosing Path Item are merged with the operation's own `parameters`; if both declare a parameter of the same name, the operation-level one wins. A `$ref` parameter is resolved against `components.parameters` the same way a schema `$ref` is resolved against `components.schemas`. An operation with no `operationId`, or with no parameters at all (after merging), gets no Params struct.
* **No JSON tags**: Params struct fields carry no `json` struct tag — unlike every other generated struct, these values are bound from path/query/header/cookie values, not unmarshaled from a JSON body, so a tag would be meaningless.
* **Required vs. optional**: Since there is no `,omitempty` tag to express optionality, a Params field's own type carries that signal directly: a required parameter maps to the plain resolved type (e.g. `UserId string`); a non-required parameter maps to a pointer to it, using the same pointerability rule as nullable schema fields (`isPointerable` — scalars and named struct/enum types only, never `map[string]any`, a slice, or `any`). A path parameter (`"in": "path"`) is always treated as required, regardless of its declared `required` value, since the OpenAPI spec mandates that path parameters are required.
* **Field naming & ordering**: Same as schema fields — PascalCase(parameter name), alphabetical by the parameter's (JSON) name.
* **Type resolution**: A parameter's `schema` resolves exactly like a property's schema (same `$ref`/inline/array/enum handling); a parameter with no `schema` maps to `any`.
